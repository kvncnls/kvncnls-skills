...
This is the complete **Product Judgement** orchestration Skill as one document. It coordinates the four foundational Skills; for a holistic audit, load this bundle together with [`bundles/focal.md`](./focal.md), [`bundles/compass.md`](./compass.md), [`bundles/flywheel.md`](./flywheel.md), and [`bundles/soul.md`](./soul.md), or install the full folder set with the Skills CLI.

**How to use it**
- **Claude Code**—install the `product-judgement/` folder together with Focal, Compass, Flywheel, and Soul for `/product-judgement` and on-demand loading.
- **Codex (CLI)**—append this bundle and the four foundational bundles to your project's `AGENTS.md`.
- **ChatGPT**—upload all five bundles as Knowledge files or add them to a Project; invoke `/product-judgement audit this app`.
- **Cursor / Windsurf / Cline**—add all five bundles as rules files, or use the folder installation where supported.

Everything below is the orchestration Skill. The four foundational bundles supply the local methodologies, rubrics, and locked output templates it calls upon.

*This bundle is synchronized manually from `product-judgement/SKILL.md` on the working tree; no commit hash is asserted until commit.*
