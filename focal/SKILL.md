...
- **`review` / `critique` / `audit` (or a file, screenshot, or URL to evaluate)** → load and follow [reference/review.md](reference/review.md). It scores each discipline 0–4 against a written rubric, totals to /12 with a band, tags issues P0–P3, and closes on a One-Purpose verdict. That file defines the rubrics, the bands, and the severities—all of them, and nowhere else.
