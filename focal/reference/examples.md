...
**Screen:** CRM contact detail—a record home carrying ten co-equal regions and three identical filled primary buttons · register: hub · audience: mixed
...
- **[P1 · Disclosure]** Nothing is deferred—ten top-level regions compete at landing. A hub is allowed many destinations, so the destination count is not the failure; the failure is that three *task* surfaces are resident on a routing surface and no element anywhere sits behind a reveal. Every element is "Now" because nobody ran the triage. **Fix:** Now—identity, read-only fields, recent activity, Deals. On-demand—composers behind their header buttons, Notes, Files. Cut—the similar-contacts rail.
